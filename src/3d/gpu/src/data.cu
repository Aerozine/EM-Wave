...
int init_data(struct data *data, const char *name, int nx, int ny, double dx,
  ...
  data->dx = dx;
  data->dy = dy;
  cudaError_t err = cudaMalloc(&(data->values), nx * ny * sizeof(double));
  if (err != cudaSuccess) {
    printf("Error: Could not allocate data\n");
    return 1;
  }
  // allocation per bytes
  // must create a kern for allocation not in byte
  {
    err = cudaMemset(data->values, 0, nx * ny * sizeof(double));
    return 0;
  }
}
