...
void set_params(struct SimulationParams *params, int problem_id) {
  ...
  case 1: // small size problem
    ...
    params->nx = params->ny = params->nz = 100; // 3D requires smaller grid
    params->dt = 0.5 / (3.e8 * sqrt(1. / (params->dx * params->dx) +
                                    1. / (params->dy * params->dy) +
                                    1. / (params->dz * params->dz))); // cfl / 2
    params->nt = 500;
  ...
  case 2: // larger size problem, usable for initial scaling tests
    ...
    params->nx = params->ny = params->nz = 200;
    params->dt = 0.5 / (3.e8 * sqrt(1. / (params->dx * params->dx) +
                                    1. / (params->dy * params->dy) +
                                    1. / (params->dz * params->dz))); // cfl / 2
    params->nt = 500;
...
int main(int argc, char **argv) {
  ...
  sim_params.dt =
      ...
      (3.e8 * sqrt(1. / (sim_params.dx * sim_params.dx) +
                   1. / (sim_params.dy * sim_params.dy) +
                   1. / (sim_params.dz * sim_params.dz))); // cfl / 2
#endif

#ifndef STABILITY_STUDY
  printf("Solving problem %d:\n", problem_id);
  printf(
      " - space %gm x %gm x %gm (dx=%g, dy=%g, dz=%g; nx=%d, ny=%d, nz=%d)\n",
      sim_params.dx * sim_params.nx, sim_params.dy * sim_params.ny,
      sim_params.dz * sim_params.nz, sim_params.dx, sim_params.dy,
      sim_params.dz, sim_params.nx, sim_params.ny, sim_params.nz);
  printf(" - time %gs (dt=%g, nt=%d)\n", sim_params.dt * sim_params.nt,
         sim_params.dt, sim_params.nt);
#endif
